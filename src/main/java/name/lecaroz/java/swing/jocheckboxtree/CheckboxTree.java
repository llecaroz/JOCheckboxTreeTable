/*
 * Copyright 2007-2013 Enrico Boldrini, Lorenzo Bigagli This file is part of
 ...
 * allows to select which features to install/uninstall). If a node has some child of different
 * checking status is greyed. You can use the same constructors of JTree to instantiate a new
...
public class CheckboxTree extends JTree
{
   ...
   */
  /*
   * this inner class (commented out) is actually the Swing way, but since JTree is still based on
   * the AWT mechanism for event handling (what causes the last added listener to be invoked last)
   * we could not override the JTree event handler, so we would _always_ have that checking a node
   * causes it to be selected. We had to work around it (see initialize() and processMouseEvent()).
   */
  ...
  /*
   * temporary solution for enabling spacebar checking. Should make use of InputMaps?
   */
  ...
  /*
   * The checking model of this CheckboxTree. On the contrary of JTree and its TreeModel, we enforce
   * that this field never be null (cf. the Null Object pattern). Due to subtleties in the sequence
   * of initialization calls (JTree.setModel gets called in the constructor, causing nodes rendering
   * before field initialization and hence a null pointer exception in the default cell renderer) we
   * make it private and initialize it in the getter.
   */
  ...
  /**
   * For GUI builders. It returns a CheckboxTree with a default tree model to show something
   * interesting. Creates a CheckboxTree with visible handles, a default CheckboxTreeCellRenderer
   * and a default TreeCheckingModel.
   */
  public CheckboxTree()
  {
  ...
  /**
   * Create a CheckboxTree with visible handles, a default CheckboxTreeCellRenderer and a default
   * TreeCheckingModel. The tree is based on the specified tree model.
   */
  ...
  /**
   * Create a CheckboxTree with visible handles, a default CheckboxTreeCellRenderer and a default
   * TreeCheckingModel. The tree root is the specified tree node.
   * 
   * @param root
  ...
  /**
   * Return the TreeCheckingModel of this CheckboxTree. This method never returns null (although it
   * may return the NullTreeCheckingModel singleton).
   * 
  ...
  /**
   * @return the paths that are in the checking set and are the (upper) roots of checked trees.
   */
  ...
  private void initialize()
  {
    ...
    /*
     * the next line (commented out) is actually the Swing way, but since JTree is still based on
     * the AWT mechanism for event handling (what causes the last added listener to be invoked last)
     * we could not override the JTree event handler, so we would _always_ have that checking a node
     * causes it to be selected. We had to work around it (see processMouseEvent()).
     */
        ...
        if (row != -1) {
          ...
          if (rect != null) {
            ...
            if ((getCellRenderer()).isOnHotspot(x - rect.x, y - rect.y)) {
              ...
              if(path!=null && path.getPath()!=null && path.getPath().length!=0 && path.getPath()[path.getPath().length-1] instanceof TreeNodeObject && ((TreeNodeObject<?>)path.getPath()[path.getPath().length-1]).canBeChecked()==false) {}
              else {
                getCheckingModel().toggleCheckingPath(getPathForRow(row));
  ...
  /**
   * Set the <code>CheckboxTreeCellRenderer</code> that will be used to draw each cell.
   * 
   * @param tcl
   *          the <code>TreeCellRenderer</code> that is to render each cell
   * @throws IllegalArgumentException
   *           if the argument is not a <code>CheckboxTreeCellRenderer</code>.
   */
  @Override
  public void setCellRenderer(TreeCellRenderer tcl)
  {
    if (!(tcl instanceof CheckboxTreeCellRenderer)) {
      throw new IllegalArgumentException("The argument does not implement the CheckboxTreeCellRenderer interface: "
            + tcl);
    }
  ...
  /**
   * Co-variant method for retrieving the <code>CheckboxTreeCellRenderer</code> of this tree.
   */
  @Override
  ...
  /**
   * Set the checking model of this CheckboxTree. If the parameter is null, the checking model is
   * set to the NullTreeCheckingModel singleton.
   * 
  ...
  {
    /*
     * in case we are dealing with DefaultTreeCheckingModel, we link/unlink it from the model of
     * this tree
     */
  ...
  /**
   * Specify whether checking a node causes it to be selected, too, or else the selection is not
   * affected. The default behavior is the former.
   * 
  ...
  /**
   * @return a string representation of the tree, including the checking, enabling and greying sets.
   */
  @Override
