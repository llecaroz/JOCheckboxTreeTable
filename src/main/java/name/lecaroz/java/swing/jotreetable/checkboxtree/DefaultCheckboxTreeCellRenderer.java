 ...
 * JLabel) with a checkbox, by adding a QuadristateCheckbox to the former onto a JPanel. Both can be
 * overridden by subclasses. Note that double-clicking the label/icon of this renderer does not
 * toggle the checkbox.
 * 
 * @author boldrini
 * @author bigagli
...
public class DefaultCheckboxTreeCellRenderer extends JPanel implements CheckboxTreeCellRenderer
{
  ...
  public DefaultCheckboxTreeCellRenderer()
  {
     ...
     * 
     * this.setLayout(new FlowLayout(FlowLayout.LEFT, 0, 0)); add(this.checkBox); add(this.label);
        ...
        boolean hasFocus)
  {
    ...
    if(component==null) { 
      ...
      if (tree instanceof CheckboxTree) {
        System.out.println("getTreeCellRendererComponent:"+object.getClass().getName());
